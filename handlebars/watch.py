...
class HandlebarsEventHandler(FileSystemEventHandler):
    ...
    def get_compiled_path(self, file):
        ...
        return file.replace(self.template_dir, self.compile_dir)

    def compile(self):
        """Compile a file using handlebars"""
        logging.info("Compiling: %s", self.template_dir)

        cmd = ['handlebars', self.template_dir, '-f', self.compile_dir, '-k', 'each', '-k', 'if', '-k', 'unless']
